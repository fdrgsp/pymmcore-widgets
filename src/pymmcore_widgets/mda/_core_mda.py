...
class MDAWidget(MDASequenceWidget):
    ...
    def setValue(self, value: MDASequence) -> None:
        ...
        if ch_props := meta.get(CHANNEL_PROPERTIES_KEY):
            # the saved sequence used the feature, so turn it back on (otherwise the
            # restored values would be both invisible and inert)
            self.channels.setLightSourceVisible(True)
